    ...
    def __init__(self, capital: float = 111.55, dry_run: bool = True):
        ...
        try:
            ...
            with open(pair_file, 'r') as f:
                best_pair = json.load(f)
            sym1, sym2 = best_pair['symbol1'], best_pair['symbol2']
            logger.info(f"Loaded dynamic pair: {sym1}-{sym2}")
        except Exception as e:
            logger.warning(f"Failed to load dynamic pair: {e}. Falling back to USO-XLE.")
            sym1, sym2 = 'USO', 'XLE'
            
        pairs = [
            PairConfig(symbol1=sym1, symbol2=sym2, allocation=1.0, max_position_size=1.0),
        ]
    ...
    def connect_broker(self):
        ...
        try:
            ...
            if abs(balance['balance'] - self.capital) > 1.0:
                logger.warning(f"Balance mismatch: Expected ${self.capital}, Got ${balance['balance']}. Updating working capital.")
                
            # Dynamically update capital for compounding
            self.capital = balance['balance']
            self.portfolio.total_capital = balance['balance']
