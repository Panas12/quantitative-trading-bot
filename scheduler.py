...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler('scheduler.log', maxBytes=5*1024*1024, backupCount=3),
...
"""
    ...
    def scan_market_job(self):
        """Scheduled job to scan for the best cointegrated pair dynamically"""
        logger.info("="*70)
    ...
    def setup_schedule(self):
        ...
        auto_execute = os.getenv('AUTO_EXECUTE', 'False').lower() == 'true'
        job_func = self.execute_trades_job if auto_execute else self.check_signals_job
        
        # Daily market scan (14:45 AM Amsterdam = 8:45 AM EST)
        schedule.every().day.at("14:45").do(self.scan_market_job)
        
        # Pre-market check (9:00 AM EST = 3:00 PM Amsterdam)
        schedule.every().day.at("15:00").do(job_func)
        
        # Post-open check (9:35 AM EST = 3:35 PM Amsterdam)
        schedule.every().day.at("15:35").do(job_func)
        
        # Midday check (12:00 PM EST = 6:00 PM Amsterdam)
        schedule.every().day.at("18:00").do(job_func)
        
        # Pre-close check (3:55 PM EST = 9:55 PM Amsterdam)
        schedule.every().day.at("21:55").do(job_func)
        ...
        logger.info("  - 14:45 (2:45 PM): Market scan (8:45 AM EST)")
    ...
    def run(self):
        ...
        logger.info("Running initial check/execution...")
        auto_execute = os.getenv('AUTO_EXECUTE', 'False').lower() == 'true'
        if auto_execute:
            self.execute_trades_job()
        else:
